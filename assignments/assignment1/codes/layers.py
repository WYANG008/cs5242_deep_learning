...
class Convolution(Layer):
    ...
    def forward(self, inputs):
        ...
        C_in, C_out = self.in_channel, self.out_channel
        S, P = self.stride, self.pad
        H_out = (H + 2*P - kernel_h)// S + 1
        W_out = (W + 2*P - kernel_w)// S + 1
        out = np.zeros((C_in, C_out, H_out, W_out))
        #Img2col
        x_pad = np.pad(inputs, ((0, 0), (0, 0), (P, P), (P, P)), mode='constant')
        #Calculate the size fo the output
        i0 = np.repeat(np.arange(kernel_h), kernel_w)
        i0 = np.tile(i0, C_in)
        i1 = S * np.repeat(np.arange(H_out), W_out)
        j0 = np.tile(np.arange(kernel_w), kernel_h * C_in)
        j1 = S * np.tile(np.arange(W_out), H_out)
        i = np.int_(i0.reshape(-1, 1) + i1.reshape(1,-1))
        j = np.int_(j0.reshape(-1, 1) + j1.reshape(1, -1))
        k = np.repeat(np.arange(C_in), kernel_h * kernel_w).reshape(-1, 1)
     
        x_cols = x_pad[:, k, i, j]
        x_cols = x_cols.transpose(1, 2, 0).reshape(kernel_h * kernel_w * C_in, -1)
        res = w.reshape((C_out, -1)).dot(x_cols) + b.reshape(-1, 1)
        res = res.reshape(C_out, H_out, W_out, N)
        outputs = res.transpose(3, 0, 1, 2)
    ...
    def backward(self, in_grads, inputs):
        ...
        out_grads = None
        N, _, H_out, W_out = in_grads.shape
        C_in, C_out = self.in_channel, self.out_channel
        w, b = self.weights, self.bias
        kernel_h, kernel_w = self.kernel_h, self.kernel_w
        S, P = self.stride, self.pad
        x_pad = np.pad(inputs, ((0, 0), (0, 0), (P, P), (P, P)), mode='constant')
        out_grads = np.zeros_like(x_pad)
        ...
        dout_reshaped = in_grads.transpose(1, 2, 3, 0).reshape(C_out, -1)
        self.w_grad = dout_reshaped.dot(x_cols.T).reshape(w.shape)

        dx_cols = w.reshape(C_out, -1).T.dot(dout_reshaped)

        dx_cols_reshaped = dx_cols.reshape(C_in * kernel_h * kernel_w, -1, N)
        dx_cols_reshaped = dx_cols_reshaped.transpose(2, 0, 1)
        np.add.at(out_grads, (slice(None), k, i, j),dx_cols_reshaped)
        if P != 0:
            out_grads = out_grads[:, :, P:-P, P:-P]
...
class Pooling(Layer):
    ...
    def forward(self, inputs):
        ...
        P, S = self.pad, self.stride
        pool_h, pool_w = self.pool_height, self.pool_width
        H_out = (H+2*P-pool_h)//S+1
        W_out = (W+2*P-pool_w)//S+1
        x_pad = np.pad(inputs, ((0, 0), (0, 0), (P, P), (P, P)), mode='constant')

        outputs = np.zeros((N,C_in,H_out,W_out))
        for i in range(H_out):
            for j in range(W_out):
                x_masked = x_pad[:,:,i*S : i*S+pool_h, j*S : j*S+pool_w]
                if self.pool_type == "max":
                    outputs[:,:,i,j] = np.max(x_masked, axis=(2,3)) 
                elif self.pool_type == "avg":
                    outputs[:,:,i,j] = np.average(x_masked, axis=(2,3)) 
    ...
    def backward(self, in_grads, inputs):
        ...
        P, S = self.pad, self.stride
        pool_h, pool_w = self.pool_height, self.pool_width
        H_out, W_out = in_grads.shape[2], in_grads.shape[3]
        x_pad = np.pad(inputs, ((0, 0), (0, 0), (P, P), (P, P)), mode='constant')
        ...
        for i in range(H_out):
         for j in range(W_out):
            ...
            if self.pool_type == "max":
                ...
                out_grads[:,:,i*S : i*S+pool_h, j*S : j*S+pool_w] += temp_binary_mask * (in_grads[:,:,i,j])[:,:,None,None]
            if self.pool_type == "avg":
                out_grads[:,:,i*S : i*S+pool_h, j*S : j*S+pool_w] += (in_grads[:,:,i,j])[:,:,None,None]/(pool_h*pool_h)
...
class Dropout(Layer):
    ...
    def forward(self, inputs):
        ...
        if self.training:
            self.mask = np.random.binomial(1, 1-p, (inputs.shape)) /(1 - p)
